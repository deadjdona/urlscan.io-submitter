...
- Added emoji synonyms/aliases for all CLI parameters (e.g. `-🎯` for `--domain`, `-🚀` for `--workers`) just for lulz!
...
- Added multi-threading concurrency with the `-w` (`--workers`) argument via `ThreadPoolExecutor` for parallel bulk scans.
- Added intelligent API rate-limiting handling by parsing urlscan.io `X-Rate-Limit-Reset-After` headers on HTTP 429 errors.
