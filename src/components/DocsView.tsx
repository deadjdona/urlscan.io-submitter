 ...
 * - Feature highlights (smart rate limiting, multi-threaded, matrix generation, IP submissions)
...
export default function DocsView() {
  ...
  const [activePlaybook, setActivePlaybook] = useState<'recon' | 'ips' | 'phishing' | 'stealth' | 'automation'>('recon');
  ...
  return (
    <div className="space-y-8 max-w-7xl mx-auto pb-12">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <div className="flex items-start gap-3.5 p-4 rounded-2xl bg-white border border-gray-200 shadow-xs">
          ...
          <div>
            ...
            <span className="text-xs text-gray-600 leading-relaxed block">
              Auto-pauses based on <code className="bg-gray-100 text-gray-800 px-1 py-0.5 rounded font-mono text-[11px]">X-Rate-Limit-Reset-After</code> headers.
            </span>
          </div>
        </div>
        ...
        <div className="flex items-start gap-3.5 p-4 rounded-2xl bg-white border border-gray-200 shadow-xs">
          ...
          <div>
            <strong className="block text-gray-900 text-sm font-bold mb-1">Multi-threaded</strong>
            <span className="text-xs text-gray-600 leading-relaxed block">
              Dispatch bulk domains concurrently with worker thread pooling (<code className="bg-gray-100 text-gray-800 px-1 py-0.5 rounded font-mono text-[11px]">-w</code> flag).
            </span>
          </div>
        </div>
        ...
        <div className="flex items-start gap-3.5 p-4 rounded-2xl bg-white border border-gray-200 shadow-xs">
          ...
          <div>
            ...
            <span className="text-xs text-gray-600 leading-relaxed block">
              Expands targets across 140+ common subdomains, root/www, and HTTP/HTTPS.
            </span>
          </div>
        </div>
        ...
        <div className="flex items-start gap-3.5 p-4 rounded-2xl bg-white border border-gray-200 shadow-xs">
          ...
          <div>
            <strong className="block text-gray-900 text-sm font-bold mb-1">IP & DNS Resolution</strong>
            <span className="text-xs text-gray-600 leading-relaxed block">
              Resolve subdomain DNS A-records and submit direct IP targets (<code className="bg-gray-100 text-gray-800 px-1 py-0.5 rounded font-mono text-[11px]">-I</code> flag).
            </span>
          </div>
        </div>
      </div>
        ...
        <div className="lg:col-span-5 space-y-8">
          ...
          <div className="bg-white rounded-2xl border border-gray-200 shadow-xs p-6 space-y-4">
            ...
            <div className="bg-[#0D1117] rounded-xl p-4 text-[12px] font-mono text-gray-300 shadow-inner space-y-4 overflow-x-auto border border-gray-800">
              <pre className="text-cyan-400 font-mono text-[8px] sm:text-[9.5px] leading-[1.12] select-none border-b border-gray-800/80 pb-3">
...
\\______________________________________________________________________/`}
          ...
          <div className="bg-white rounded-2xl border border-gray-200 shadow-xs p-6 space-y-4">
            ...
            <div className="bg-[#0D1117] rounded-xl p-4 text-[12px] font-mono text-gray-300 shadow-inner overflow-x-auto border border-gray-800">
              ...
              <br />
              <p className="text-gray-500"># Submit URL or IP with backoff & error handling</p>
              <p className="text-gray-300">res = submit_to_urlscan(</p>
              <p className="text-gray-300 pl-4"><span className="text-green-300">"https://123.21.33.22/"</span>,</p>
          ...
          <div className="bg-white rounded-2xl border border-gray-200 shadow-xs p-6 space-y-5">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-gray-100 pb-4">
              ...
              <div className="flex items-center gap-1 bg-gray-100 p-1 rounded-xl text-xs font-medium">
                ...
                <button
                  onClick={() => setActivePlaybook('ips')}
                  className={`px-2.5 py-1 rounded-lg transition-all cursor-pointer ${
                    activePlaybook === 'ips' ? 'bg-white text-blue-600 shadow-2xs font-bold' : 'text-gray-600 hover:text-gray-900'
                  }`}
                >
                  IPs & DNS
                </button>
            ...
            {activePlaybook === 'ips' && (
              <div className="space-y-4 text-xs">
                <div className="p-4 rounded-xl bg-purple-50/50 border border-purple-100 space-y-2">
                  <div className="flex items-center justify-between">
                    <strong className="text-purple-900 font-bold">2. Subdomain IP Resolution & Direct IP Submissions</strong>
                    <button
                      onClick={() => handleCopy('urlscan-submit -d target.com -p both -s both -x -I -j ip_results.json', 'pb-ips')}
                      className="text-xs text-purple-700 hover:text-purple-900 flex items-center gap-1 font-medium cursor-pointer"
                    >
                      {copiedSection === 'pb-ips' ? <Check size={12} /> : <Copy size={12} />}
                      {copiedSection === 'pb-ips' ? 'Copied' : 'Copy'}
                    </button>
                  </div>
                  <p className="text-gray-600 leading-relaxed">
                    Automatically resolves DNS A-records for subdomains and submits both the hostnames and their direct IP addresses (<code className="font-mono bg-purple-100 text-purple-900 px-1 rounded">http://123.21.33.22/</code> and <code className="font-mono bg-purple-100 text-purple-900 px-1 rounded">https://123.21.33.22/</code>). Direct IP targets can also be submitted directly via <code className="font-mono bg-purple-100 text-purple-900 px-1 rounded">-d 123.21.33.22 -p both</code>.
                  </p>
                  <pre className="bg-[#0D1117] text-yellow-300 p-3 rounded-lg font-mono overflow-x-auto text-[11px]">
# Resolve subdomain IPs and submit http:// & https:// IP variants:
urlscan-submit -d target.com -p both -s both -x -I -j ip_results.json

# Submit direct IP address:
urlscan-submit -d 123.21.33.22 -p both
            ...
            {activePlaybook === 'phishing' && (
              <div className="space-y-4 text-xs">
                <div className="p-4 rounded-xl bg-purple-50/50 border border-purple-100 space-y-2">
                  <div className="flex items-center justify-between">
                    <strong className="text-purple-900 font-bold">3. Bulk Phishing Campaign Triage</strong>
                    <button
            ...
            {activePlaybook === 'stealth' && (
              <div className="space-y-4 text-xs">
                <div className="p-4 rounded-xl bg-amber-50/50 border border-amber-100 space-y-2">
                  <div className="flex items-center justify-between">
                    <strong className="text-amber-900 font-bold">4. Evasion & Traffic Spoofing</strong>
                    <button
            ...
            {activePlaybook === 'automation' && (
              <div className="space-y-4 text-xs">
                <div className="p-4 rounded-xl bg-emerald-50/50 border border-emerald-100 space-y-2">
                  <div className="flex items-center justify-between">
                    <strong className="text-emerald-900 font-bold">5. SIEM / SOAR Pipeline Ingestion</strong>
                    <button
...
urlscan-submit -f daily_domain_feed.txt -j daily_siem_feed.json -w 12 --tags "siem-cron" -V private
          ...
          <div className="bg-white rounded-2xl border border-gray-200 shadow-xs p-6 space-y-4">
            ...
            <div className="space-y-3 text-xs">
              ...
              <div className="border border-gray-200 rounded-xl overflow-hidden">
                <button
                  onClick={() => toggleFaq('ips')}
                  className="w-full p-3.5 bg-gray-50/70 hover:bg-gray-100 transition-colors flex items-center justify-between text-left font-bold text-gray-900 cursor-pointer"
                >
                  <span className="flex items-center gap-2">
                    <Server size={15} className="text-purple-500" />
                    <span>How does IP submission and subdomain DNS resolution work?</span>
                  </span>
                  {openFaq['ips'] ? <ChevronDown size={16} /> : <ChevronRight size={16} />}
                </button>
                {openFaq['ips'] && (
                  <div className="p-4 bg-white space-y-2 text-gray-600 border-t border-gray-200 leading-relaxed">
                    <p>
